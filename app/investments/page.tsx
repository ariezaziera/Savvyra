...
type GoldPrice = { pricePerGram: number | null; pricePerOz: number | null; error?: string };
...
const STATUS_COLORS: Record<string, string> = {
  ACTIVE: "text-[#8EE3B5] bg-[#8EE3B5]/15", MATURED: "text-[#C4B5FD] bg-[#C4B5FD]/15", WITHDRAWN: "text-[#FBD38D] bg-[#FBD38D]/15",
};
...
const emptyForm = {
  name: "", platform: "", type: "General", subType: "",
  principalAmount: "", currentValue: "", monthlyContribution: "",
  returnRate: "", startDate: "", maturityDate: "", status: "ACTIVE", note: "",
  goldGrams: "", goldBuyPricePerGram: "", goldCurrentPricePerGram: "", goldSellingPricePerGram: "",
};

// ── Gold Form ────────────────────────────────────────────────────
function GoldForm({ form, setForm, marketPrice }: { form: any; setForm: any; marketPrice: number | null }) {
  const f = (k: string, v: string) => setForm((p: any) => ({ ...p, [k]: v }));
  const grams   = parseFloat(form.goldGrams) || 0;
  const buyPx   = parseFloat(form.goldBuyPricePerGram) || 0;
  const curPx   = parseFloat(form.goldCurrentPricePerGram) || 0;
  const sellPx  = parseFloat(form.goldSellingPricePerGram) || 0;
  const totalPurchased = grams * buyPx;
  const currentVal     = grams * curPx;
  const sellingVal     = grams * sellPx;
  const gainLoss       = sellingVal - totalPurchased;
    ...
    <div className="space-y-4">
      ...
      {marketPrice && (
        <div className="flex items-center gap-2.5 rounded-2xl border border-[#FBD38D]/20 bg-[#FBD38D]/8 px-4 py-2.5">
          ...
          <p className="text-xs text-[#FBD38D]">
            Market reference: <span className="font-bold">{fmt(marketPrice)}/gram</span> (MYR) — platform prices may vary
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Field label="Platform">
          <Select value={form.platform || "MAE MIGA"} onChange={(e: any) => f("platform", e.target.value)} options={GOLD_PLATFORMS}/>
        </Field>
        <Field label="Name / Label">
          <Input value={form.name} onChange={(e: any) => f("name", e.target.value)} placeholder="e.g. MAE MIGA Gold"/>
        </Field>
        <Field label="Grams Owned *">
          <Input value={form.goldGrams} onChange={(e: any) => f("goldGrams", e.target.value)} placeholder="e.g. 5.00" type="number"/>
        </Field>
        <Field label="Avg Buy Price / gram (RM) *">
          <Input value={form.goldBuyPricePerGram} onChange={(e: any) => f("goldBuyPricePerGram", e.target.value)} placeholder="e.g. 380.00" type="number"/>
        </Field>
        <Field label="Current Price / gram (RM)">
          <Input value={form.goldCurrentPricePerGram} onChange={(e: any) => f("goldCurrentPricePerGram", e.target.value)} placeholder="Check your app" type="number"/>
        </Field>
        <Field label="Selling / Buyback Price / gram (RM)">
          <Input value={form.goldSellingPricePerGram} onChange={(e: any) => f("goldSellingPricePerGram", e.target.value)} placeholder="Platform buyback rate" type="number"/>
        </Field>
      </div>
          ...
          {sellPx > 0 && (
            <>
              ...
              <div className="flex justify-between border-t border-white/10 pt-2">
                <span className="text-white/50">Gain / Loss if sold now</span>
      ...
      <Field label="Note (optional)">
        <Input value={form.note} onChange={(e: any) => f("note", e.target.value)} placeholder="e.g. started Jan 2024"/>
      </Field>
    </div>
  );
}
...
// ── General Form ─────────────────────────────────────────────────
function GeneralForm({ form, setForm }: { form: any; setForm: any }) {
  const f = (k: string, v: string) => setForm((p: any) => ({ ...p, [k]: v }));
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
...
// ── Gold Card ────────────────────────────────────────────────────
function GoldCard({ inv, onEdit, onDelete, onUpdatePrice }: {
  inv: Investment; onEdit: () => void; onDelete: () => void; onUpdatePrice: (id: string, cur: number, sell: number) => void;
}) {
    ...
    <div className="relative overflow-hidden rounded-3xl border border-[#FBD38D]/20 bg-[#FBD38D]/5 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.35)]">
      ...
      <button className="w-full flex items-center justify-between px-5 py-4 text-left" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/40">{inv.platform} · {grams}g gold</p>
          </div>
        </div>
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t border-white/8 pt-4 space-y-4">
          ...
          {sellingVal > 0 && (
            <div className={`flex items-center gap-2.5 rounded-2xl px-4 py-2.5 border ${worthSelling ? "border-[#8EE3B5]/25 bg-[#8EE3B5]/10" : "border-[#FF8C8C]/25 bg-[#FF8C8C]/10"}`}>
              ...
              <p className={`text-xs font-medium ${worthSelling ? "text-[#8EE3B5]" : "text-[#FF8C8C]"}`}>
                {worthSelling ? `Selling now nets you +${fmt(sellingVal - totalPurch)} profit` : `Selling now = loss of ${fmt(Math.abs(sellingVal - totalPurch))}`}
              </p>
            </div>
          )}
          ...
          {/* Update price inline */}
          {!updating ? (
            <button onClick={() => setUpdating(true)} className="flex items-center gap-2 text-xs text-white/35 hover:text-[#FBD38D] transition">
            ...
            <div className="space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Current Price / gram (RM)"><Input value={curPrice} onChange={(e: any) => setCurPrice(e.target.value)} placeholder="From your app" type="number"/></Field>
                <Field label="Selling / Buyback Price / gram (RM)"><Input value={sellPrice} onChange={(e: any) => setSellPrice(e.target.value)} placeholder="Platform buyback" type="number"/></Field>
              </div>
              <div className="flex gap-2">
                ...
                <button onClick={() => { onUpdatePrice(inv.id, parseFloat(curPrice) || 0, parseFloat(sellPrice) || 0); setUpdating(false); }}
                  className="flex-1 rounded-full bg-[#FBD38D]/20 border border-[#FBD38D]/30 py-2 text-xs font-semibold text-[#FBD38D] hover:bg-[#FBD38D]/35 transition">
                  Save Prices
                </button>
              </div>
            </div>
...
export default function InvestmentsPage() {
  ...
  const [tab, setTab]                 = useState<"all" | "gold" | "other">("all");
  ...
  const handleAdd = async () => {
    ...
    if (res.ok) { await fetchInvestments(); setForm(emptyForm); setShowForm(false); showToast("Investment added ✅"); }
    setSaving(false);
  };
  ...
  const handleUpdatePrice = async (id: string, goldCurrentPricePerGram: number, goldSellingPricePerGram: number) => {
    const res = await fetch(`/api/investments/${id}`, {
      ...
      body: JSON.stringify({ goldCurrentPricePerGram, goldSellingPricePerGram }),
    });
    if (res.ok) { await fetchInvestments(); showToast("Prices updated ✅"); }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-3xl border border-white/15 bg-[#1a1035] p-6 shadow-[0_24px_64px_rgba(0,0,0,0.6)]">
            ...
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div><p className="font-bold text-white">Delete this investment?</p><p className="text-sm text-white/45 mt-1.5">This cannot be undone.</p></div>
              <div className="flex gap-3 w-full">
                <button onClick={() => setConfirmDeleteId(null)} className="flex-1 rounded-full border border-white/15 bg-white/5 py-2.5 text-sm text-white/60 hover:text-white transition">Cancel</button>
                <button onClick={() => handleDelete(confirmDeleteId)} className="flex-1 rounded-full bg-[#FF8C8C]/20 border border-[#FF8C8C]/30 py-2.5 text-sm font-semibold text-[#FF8C8C] hover:bg-[#FF8C8C]/35 transition">Delete</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="relative z-10">
        <div className="mb-8 flex items-start justify-between">
          <div>
            ...
            <p className="mt-1.5 text-sm text-white/50">Track your portfolio — gold, stocks, ASNB and more.</p>
          </div>
          <button onClick={() => { setForm(emptyForm); setShowForm(!showForm); }}
            className="flex items-center gap-2 rounded-2xl bg-linear-to-r from-[#6A49FA] to-[#9B7FFF] px-4 py-2.5 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(106,73,250,0.40)] transition hover:scale-[1.03] active:scale-[0.97] mt-2">
            <Plus size={16}/> Add
          </button>
        </div>

        {/* Market gold price banner */}
        {marketPrice.pricePerGram && (
          <div className="mb-4 flex items-center gap-3 rounded-2xl border border-[#FBD38D]/20 bg-[#FBD38D]/8 px-4 py-3">
            ...
            <p className="text-xs text-[#FBD38D]">
              Live market gold: <span className="font-bold">{fmt(marketPrice.pricePerGram)}/gram</span> · <span className="opacity-60">This is a reference rate — your platform price may differ</span>
            </p>
          </div>
        )}
        ...
        <div className="flex gap-2 mb-5">
          {([["all","All"], ["gold","Gold 🪙"], ["other","Stocks & Others"]] as const).map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)}
              className={`rounded-full px-4 py-2 text-xs font-semibold transition-all ${tab === key ? "bg-[#6A49FA]/40 text-[#C4B5FD] border border-[#6A49FA]/40" : "border border-white/10 text-white/40 hover:text-white/70"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <AnimatePresence>
          {showForm && (
            <motion.div initial={{ opacity:0, y:-10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-10 }}
              ...
              <div className="mb-4">
                <Field label="Type">
                  <Select value={form.type} onChange={(e: any) => setForm((p) => ({ ...p, type: e.target.value }))} options={TYPES}/>
                </Field>
              </div>

              {form.type === "Gold"
                ? <GoldForm form={form} setForm={setForm} marketPrice={marketPrice.pricePerGram}/>
                : <GeneralForm form={form} setForm={setForm}/>
              }
                        ...
                        <>
                          ...
                          <div className="flex gap-2 pt-1">
                            <button onClick={() => { setEditForm((p) => ({ ...p, [inv.id]: { ...inv, startDate: inv.startDate?.split("T")[0] ?? "" } })); setEditingId(inv.id); }}
                              className="flex items-center gap-1.5 rounded-2xl bg-white/5 border border-white/10 px-3 py-2 text-xs text-white/50 hover:text-white transition"><Pencil size={12}/> Edit</button>
                            <button onClick={() => setConfirmDeleteId(inv.id)}
                              className="flex items-center gap-1.5 rounded-2xl bg-[#FF8C8C]/10 border border-[#FF8C8C]/20 px-3 py-2 text-xs text-[#FF8C8C]/70 hover:text-[#FF8C8C] transition ml-auto"><Trash2 size={12}/> Delete</button>
                          </div>
                        </>
                      ) : (
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            {[
                              ...
                              <Field key={key} label={label}>
                                <Input value={editForm[inv.id]?.[key] ?? ""} onChange={(e: any) => setEditForm((p) => ({ ...p, [inv.id]: { ...p[inv.id], [key]: e.target.value } }))} type={type}/>
                              </Field>
                            ))}
                          </div>
                          <div className="flex gap-3 pt-1">
                            ...
                            <button onClick={async () => { setSaving(true); const res = await fetch(`/api/investments/${inv.id}`, { method:"PATCH", headers:{"Content-Type":"application/json"}, body: JSON.stringify(editForm[inv.id]) }); if (res.ok) { await fetchInvestments(); setEditingId(null); showToast("Updated ✅"); } setSaving(false); }} disabled={saving}
