...
export default function SalaryPage() {
  ...
  // ── Allowance helpers ──
  const addAllowance = () => {
    // FIX: include isReimbursement: false in default
        ...
        <AnimatePresence mode="wait">
          ...
          {tab === "calculator" && (
            <motion.div key="calc" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-5">
              ...
              {/* ── ALLOWANCES — redesigned to match Custom Deductions style ── */}
              <SectionCard title="Allowances">
                <div className="space-y-1">
                    ...
                    <div key={i}>
                      ...
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          placeholder="e.g. Transport, Meal, Housing"
                          type="text"
                      ...
                      {/* Toggle pills row */}
                      <div className="flex items-center gap-2 pl-1 pt-2 pb-3">
                        {/* Cut on absent */}
                        <button
                          type="button"
                          ...
                          className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-all border ${
                            a.cutOnAbsent
                              ? "border-[#FF8C8C]/40 bg-[#FF8C8C]/15 text-[#FF8C8C]"
                              : "border-white/10 bg-white/5 text-white/35 hover:text-white/55 hover:bg-white/8"
                          }`}
                        >
                          <span className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${a.cutOnAbsent ? "bg-[#FF8C8C]" : "bg-white/20"}`} />
                          Cut on absent
                        </button>

                        {/* Receipt-based / isReimbursement */}
                        <button
                          type="button"
                          onClick={() => updateAllowance(i, "isReimbursement", !a.isReimbursement)}
                          className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-all border ${
                            a.isReimbursement
                              ? "border-[#FBD38D]/40 bg-[#FBD38D]/15 text-[#FBD38D]"
                              : "border-white/10 bg-white/5 text-white/35 hover:text-white/55 hover:bg-white/8"
                          }`}
                        >
                          <span className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${a.isReimbursement ? "bg-[#FBD38D]" : "bg-white/20"}`} />
                          Receipt-based claim
                        </button>
                      </div>

                      {/* Divider between entries */}
                      {i < inputs.allowances.length - 1 && (
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs text-[#C4B5FD] hover:text-white transition pt-1"
                  >
                ...
                {/* Contextual info box */}
                <div className="mt-4 rounded-2xl border border-white/8 bg-white/4 px-4 py-3.5 space-y-2 text-xs leading-relaxed">
                  <p className="font-medium text-white/55">How these toggles work:</p>
                  <div className="space-y-1.5 text-white/38">
                    <p>
                      <span className="text-[#FF8C8C] font-medium">Cut on absent</span>
                      {" "}— allowance akan dipotong secara berkadar jika ada unpaid leave.
                    </p>
                    <p>
                      <span className="text-[#FBD38D] font-medium">Receipt-based claim</span>
                      {" "}— aktif ini jika elaun dibayar berdasarkan resit yang dikemukakan (contoh: tol, parking, mileage claim).
                      Ia akan dikecualikan dari pengiraan SOCSO, EIS, dan EPF mengikut peraturan PERKESO/KWSP.
                    </p>
                    <p className="text-white/25 pt-0.5">
                      💡 Elaun tetap bulanan (transport RM200, makan RM150) = biarkan kedua-dua toggle ini mati.
                    </p>
                  </div>
                </div>
              </SectionCard>
              ...
              <div className="relative overflow-hidden rounded-3xl border border-[#6A49FA]/30 bg-linear-to-br from-[#6A49FA]/20 to-[#C4B5FD]/10 p-6 backdrop-blur-2xl shadow-[0_8px_40px_rgba(106,73,250,0.25)]">
                ...
                <div className="space-y-3 text-sm">
                  {[
                    ...
                    breakdown.allowanceTotal > 0 && { label: "Allowances", value: breakdown.allowanceTotal, color: "text-[#8EE3B5]" },
                    breakdown.reimbursementTotal > 0 && { label: "Reimbursements", value: breakdown.reimbursementTotal, color: "text-[#FBD38D]" },
                    ...
                    <div key={row.label} className="flex justify-between">
                      ...
                      <span className={row.color}>{row.value < 0 ? `− ${fmt(Math.abs(row.value))}` : fmt(row.value)}</span>
                    </div>
                  ))}
                </div>
