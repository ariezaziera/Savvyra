...
// ── Glass row (dark glass — used everywhere now) ──
...
function ProfileTab({ glass }: { glass: boolean }) {
  ...
  return (
    <div className="space-y-4">
      {/* Avatar — always dark glass style */}
      ...
      {/* Notifications — always dark glass style */}
      ...
      <button onClick={() => signOut({ callbackUrl: "/login" })}
        className="w-full flex items-center justify-center gap-2 rounded-2xl py-3 text-sm font-bold transition-all border border-[#FF8C8C]/25 bg-[#FF8C8C]/8 text-[#FF8C8C] hover:bg-[#FF8C8C]/15">
...
function SecurityTab({ glass }: { glass: boolean }) {
  return (
    <div className="space-y-3">
      <p className="text-[10px] font-bold uppercase tracking-widest mb-1 text-white/30">Account Security</p>
      <GlassRow icon={Lock}        label="Change Password" value="Update your login credentials" onClick={() => {}} iconColor="text-[#C4B5FD]" iconBg="bg-[#C4B5FD]/15"/>
      <GlassRow icon={Fingerprint} label="Biometric Login"  value="Face ID / Fingerprint"         soon iconColor="text-[#8EE3B5]" iconBg="bg-[#8EE3B5]/15" toggle toggled={false}/>
      <GlassRow icon={Shield}      label="Two-Factor Auth"  value="Extra layer of protection"     soon iconColor="text-[#FBD38D]" iconBg="bg-[#FBD38D]/15"/>
...
function SupportTab({ glass }: { glass: boolean }) {
  ...
  const inputCls = "w-full rounded-2xl border border-white/10 bg-white/6 px-4 py-3 text-sm text-white outline-none placeholder:text-white/25 focus:border-[#6A49FA]/60 focus:ring-2 focus:ring-[#6A49FA]/20 transition";

  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3">
        <a href="mailto:support@savvyra.app"
          className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/4 hover:bg-white/8 px-4 py-3.5 transition cursor-pointer">
          <Mail size={16} className="text-[#C4B5FD]"/>
          <div>
            <p className="text-xs font-bold text-white">Email</p>
            <p className="text-[10px] mt-0.5 text-white/35">support@savvyra.app</p>
          </div>
        </a>
        <a href="https://wa.me/60123456789" target="_blank" rel="noreferrer"
          className="flex items-center gap-3 rounded-2xl border border-[#8EE3B5]/20 bg-[#8EE3B5]/8 hover:bg-[#8EE3B5]/15 px-4 py-3.5 transition cursor-pointer">
          <MessageCircle size={16} className="text-[#8EE3B5]"/>
          <div>
            <p className="text-xs font-bold text-white">WhatsApp</p>
            <p className="text-[10px] mt-0.5 text-white/35">Chat with us</p>
          </div>
        </a>
      </div>

      <div>
        <p className="text-[10px] font-bold uppercase tracking-widest mb-3 text-white/30">FAQ</p>
        <div className="rounded-2xl overflow-hidden border border-white/8 bg-white/4">
          {FAQ.map(({ q, a }, i) => (
            <div key={i} className="border-b last:border-0 border-white/6">
              <button className="w-full flex items-center justify-between px-4 py-3.5 text-left" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                <p className="text-xs font-medium pr-4 text-white/80">{q}</p>
                {openFaq === i ? <X size={13} className="text-white/30 shrink-0"/> : <ChevronRight size={13} className="text-white/30 shrink-0"/>}
              </button>
              {openFaq === i && (
                <div className="px-4 pb-4 border-t border-white/6">
                  <p className="text-xs leading-relaxed mt-3 text-white/45">{a}</p>
                </div>
              )}
            </div>
          ))}
      ...
      <div>
        <p className="text-[10px] font-bold uppercase tracking-widest mb-3 text-white/30">Contact Us</p>
        <div className="rounded-3xl border border-white/10 bg-white/5 p-5 space-y-3">
          {sent && (
            <div className="flex items-center gap-2 rounded-2xl px-4 py-3 bg-[#8EE3B5]/15 border border-[#8EE3B5]/25">
              <Check size={14} className="text-[#8EE3B5]"/>
              <p className="text-xs font-medium text-[#8EE3B5]">Message sent! We'll reply within 24 hours.</p>
            </div>
          )}
          ...
          <button onClick={handleSend} disabled={sending || !form.name || !form.email || !form.msg}
            className="w-full rounded-full py-3 text-sm font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-gradient-to-r from-[#6A49FA] to-[#9B7FFF] text-white shadow-[0_8px_24px_rgba(106,73,250,0.40)] hover:scale-[1.02] active:scale-[0.98]">
  ...
  const content = (
    <AnimatePresence mode="wait">
      <motion.div key={tab} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-8 }} transition={{ duration:0.25, ease:"easeOut" }}>
        {tab === "profile"  && <ProfileTab  glass={true}/>}
        {tab === "security" && <SecurityTab glass={true}/>}
        {tab === "about"    && <AboutTab    glass={true}/>}
        {tab === "support"  && <SupportTab  glass={true}/>}
      </motion.div>
    </AnimatePresence>
  );
  ...
  return (
    <PageContainer>
      ...
      <div className="relative z-10">
        ...
        {/* TOP TAB BAR — both mobile & desktop */}
        <div className="mb-6 grid grid-cols-4 gap-1.5 rounded-2xl border border-white/10 bg-white/5 p-1.5 backdrop-blur-xl">
          {TABS.map(({ key, label, icon: Icon }) => (
            <button key={key} onClick={() => setTab(key)}
              className={`flex items-center justify-center gap-2 rounded-xl px-3 py-3 text-xs md:text-sm font-medium transition-all duration-200
